...
- Produces: `editorReducer(state, action)` (past/present/future, 상한 100), `useDeckEditor(projectName, initialDeck, onDeckChange, timings?) -> { deck, plan, saveState, error, canUndo, canRedo, apply, replace, undo, redo, flushSave }` (apply = 편집 반영, replace = AI 반영처럼 다음 저장을 스냅샷으로, flushSave = 보류 중 자동 저장의 즉시 실행, 성공 여부를 boolean으로 반환(2026-08-29 태스크 16 리뷰 반영: 실패 시 호출자가 내보내기를 중단할 수 있게). 언마운트 시 자동 플러시 내장, saveState에 "저장 대기" 포함), `applyTextEdit(deck, ref, text)` (모든 인라인 텍스트 수정의 단일 경로. 표 칸은 개행을 공백으로 치환해 스키마 금지와 정합), `<ChapterList>`, `<EditorScreen onEditorReady?>` (마운트 시 flushSave를 부모에 등록: Task 16의 내보내기가 쓴다). Task 13~16이 `apply`/`replace`만 부른다(직접 putDeck 금지).
...
2026-08-29 태스크 리뷰 정정: 장 전환 시 결과 패널을 리셋하는 effect를 추가한다. 이전 장의 결과가 다른 장에 반영되는 오귀속 쓰기(리뷰 발견)의 수리.
...
- Produces: 내보내기 버튼(슬라이드가 있을 때만): 보류 중 자동 저장을 먼저 플러시하고(실패 시 내보내기 중단, 2026-08-29 태스크 16 리뷰 반영), 스냅샷을 먼저 남기고(결정 1의 ④) 내보낸 뒤 파일 경로를 보여준다. `<RecoveryScreen project onBack />`: 스냅샷 목록에서 복원(확인 대화 포함). needs_recovery 프로젝트는 이 화면으로만 진입하고, 정상 프로젝트도 머리글의 "스냅샷 복구"로 들어올 수 있다(설계서 7.2 복구 메뉴).
...
const preset = { /* EditorScreen.test.tsx의 preset 픽스처와 동일 */ } as unknown as Preset;

// 편집 탭 미리보기(실측 결과)도 EditorScreen.test.tsx 픽스처를 재사용한다
const plan: RenderPlan = { /* EditorScreen.test.tsx의 plan 픽스처와 동일 (chapter_id "c1"의 불릿 "하나") */ } as RenderPlan;
...
it("복구가 필요한 프로젝트는 복구 화면으로 진입한다", async () => {
  ...
  // RecoveryScreen 고유 산출(빈 스냅샷 목록 문구)로 단언한다 (2026-08-29 태스크 16 리뷰 정정)
...
it("마지막 편집 저장에 실패하면 내보내기를 중단한다", async () => {
  ...
  await userEvent.clear(box);
  await userEvent.type(box, "고침{Enter}");
  // 기본 timings(1.2초 디바운스)에서는 자동 저장이 발화하지 않고, 아래 클릭이 flushSave로
  // 미저장분을 감지해 putDeck을 호출한다
...
(2026-08-29 태스크 16 리뷰 반영으로 추가된 4번째 테스트는 EditorScreen도 같은 실패로 자체 오류 배너를 띄워
`role="alert"`가 동시에 2개 존재하므로 `findByRole("alert")` 단일 조회 대신 중단 안내 문구와 role 컨테이너를
함께 확인해 정밀화했다. 실제 픽스처 전문은 `frontend/src/screens/ProjectView.test.tsx` 참조.)
...
- 상태 추가: `const [exportPath, setExportPath] = useState("");`, `const [exporting, setExporting] = useState(false);`, `const [showRecovery, setShowRecovery] = useState(false);`, `const flushEditor = useRef<null | (() => Promise<boolean>)>(null);` (import에 `useRef` 추가)
