...
## 구현 중 계획과 달라진 점 (2026-09-01, 태스크 리뷰와 브랜치 최종 리뷰 반영)

- 태스크 1: `display: grid`는 `.field`가 아니라 그 안의 `label`에 둔다(label이 input을 감싸므로 거기에 있어야 세로로 쌓인다. 계획서 CSS가 부정확했다).
- 태스크 2: 경로 조각 제거는 `Path(...).name`이 아니라 `PureWindowsPath(...).name`으로 한다(실행 OS와 무관하게 역슬래시도 분리자로 처리). OpenAPI와 TS 타입 재생성은 라우트를 추가한 커밋마다 수행했다(각 커밋이 `test_openapi`를 통과하게 하기 위해서다).
- 태스크 2와 3(브랜치 최종 리뷰): 업로드 라우트는 `X-Requested-With: SlideCaptain` 헤더가 없으면 400으로 거절하고 클라이언트가 이 헤더를 붙인다. 원시 본문 POST는 브라우저의 단순 요청이라 다른 사이트의 페이지가 사전 확인 없이 자료를 써 넣을 수 있었기 때문이다(커스텀 헤더가 있으면 사전 확인이 강제되고 이 서버는 OPTIONS에 405로 답한다).
- 태스크 3: 덮어쓰기 확인 문구는 "같은 이름의 자료가 이미 있습니다: 파일명. 덮어쓸까요?"로, 추가 0개일 때 안내는 "추가한 자료가 없습니다."로 정했다(조사 병기 제거와 성공 문구 오해 방지. 태스크 3 리뷰).
- 태스크 4: 상태 줄에 `role="status"`를 두고 `describeStatus`는 비공개로 했다(태스크 4 프런트 리뷰). 오류 문구의 stderr 발췌에서 이메일 패턴을 가린다(최종 리뷰).
