...
태스크마다 구현자와 독립 리뷰어를 분리했고, 리뷰어는 구현 직전 커밋에 새 테스트를 이식해 실제로 실패하는지 재실증했다(전 태스크에서 구현자의 RED 건수와 일치).

| 태스크 | 커밋 | 리뷰 판정 | 반영 |
|---|---|---|---|
| A1 저장소 | a78922b | 승인 (minor 2, nit 2) | DeckConflict 문구 상수화는 A2 에서. `write_source` 검증 순서 변경은 현행 유지 |
| A2 API | 00e884d | 승인 (minor 1, nit 3) | Host 스푸핑과 헤더 없음 겹침 테스트를 f10be3f 에서 추가. If-Match 와일드카드와 IPv6 루프백은 이월표 |
| A3 검증 | 0288b47 | 승인 (minor 2, nit 2) | 주석 인용 정정을 f10be3f 에서. 필드 단위 영어 문구는 이월표 |
| A4 이름 | ef2c767 | 승인 (minor 2, nit 2) | 대소문자 충돌 비교의 NFC 정규화를 f10be3f 에서 추가(테스트 포함). 업로드 응답 filename 은 이월표 |
| A5a 클라이언트와 편집기 | 6fcd0e6 | 승인 (nit 3) | 412 응답에 ETag 가 없다는 참고를 A5b 설계에 반영 |
| A5b 화면 통합 | ef6cc7a | 수정 후 승인 (major 2, minor 2, nit 3) | 219f653: 최초 승인 PUT 의 412, 복구 화면 복귀 뒤 배너 잔존, 자료 탭 이동의 중복 배너, returnValue. 승인 루프 중단 뒤 뒤 순번 장 표시(F4)와 needs_recovery 경로(F6)는 이유를 적고 이월 |
| 묶음 최종 | 6b85011..a51b160 | 수정 후 승인 (major 1, minor 1, nit 1) | 17ad619: 억제 플래그를 플러시 직전에 재설정, 편집기 충돌 힌트로 일반 배너 생략. nit(219f653 커밋 메시지에 중점 1자)는 이력 재작성을 피해 기록만 남긴다 |

실행 중 발견한 환경 함정: worktree 에서 pytest 를 그냥 돌리면 빌린 가상환경의 editable 설치가 main 클론을 가리켜 이 작업 공간의 코드가 검증되지 않는다. `PYTHONPATH=<worktree>/backend` 를 붙여야 한다(CLAUDE.md 관례에 추가).
