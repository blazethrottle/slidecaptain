...
- 프로바이더 호출 구성은 스파이크로 검증된 조합을 쓴다: `tools=[]`, `setting_sources=[]`, `max_turns=2`, `output_format={"type": "json_schema", "schema": ...}` (아래 "착수 전 실증 결과") (2026-08-28 정정: 당초 스파이크의 1은 트리비얼 프롬프트에서만 통과. 실제 생성 프롬프트는 구조화 출력 정리 턴을 포함해 2가 실측 최소값)
