...
- 프로바이더 호출 구성은 실증으로 검증된 조합을 쓴다: `tools=[]`, `setting_sources=[]`, `max_turns=2`, `output_format={"type": "json_schema", "schema": ...}` (아래 "착수 전 실증 결과") (2026-08-28 정정: 당초 스파이크의 1은 트리비얼 프롬프트에서만 통과. 실제 생성 프롬프트는 구조화 출력 정리 턴을 포함해 2가 실측 최소값)
...
- 실험 조건: `ANTHROPIC_API_KEY` 부재 단언, `CLAUDE_CODE_*` 환경 변수 제거(일반 터미널 조건과 일치), `tools=[]`, `setting_sources=[]`, `max_turns=1`, json_schema 구조화 출력 (2026-08-28 정정: 이 조건의 1은 트리비얼 프롬프트에서만 통과했다. 실제 생성 프롬프트는 구조화 출력 정리 턴을 포함해 2가 실측 최소값이며, 태스크 11 스모크에서 이 결함이 드러나 상향했다)
...
12. **재호출 프롬프트에 직전 산출물을 동봉한다** (2026-08-28 적대 리뷰 반영): 매 호출이 새 세션(호출 간 컨텍스트 비공유)이라 모델의 컨텍스트에 직전 응답이 존재하지 않는다. 형식 재시도 프롬프트에는 실패한 응답 원문(앞 2,000자)을, 축약 프롬프트에는 직전 초안 JSON을 포함시켜, 1회 한도의 재호출이 실제로 직전 산출물을 딛고 개선하게 한다. 장별 프롬프트에는 보고 정보 블록(덱 제목, 피보고자, 오늘 날짜)을 넣어 cover 생성이 날짜와 보고 대상을 지어내지 않게 한다.
