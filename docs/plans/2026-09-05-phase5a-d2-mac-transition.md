...
4. **폰트 격리**: `backend/tests/conftest.py` 에 autouse 픽스처를 두어 모든 테스트에서 폰트 설치 대상을 `tmp_path` 아래로 돌리고 `_system_font_dirs` 를 빈 목록으로 바꾼다. (2026-09-05 정정, D2-2 구현과 리뷰 실측: 초안은 `installer._user_font_dir` 자체를 교체하려 했으나 `test_fonts.py` 의 `test_user_font_dir_windows/macos/linux` 는 그 함수를 재정의하지 않고 `sys.platform` 만 바꿔 원본 분기의 반환값을 단언하므로 함수를 갈아치우면 그 세 테스트가 깨진다. 실제 구현은 `Path.home` 을 임시 폴더로 패치해 macOS 와 Linux 분기가 원본 로직대로 돌면서 결과만 격리되게 하고, win32 분기는 `Path.home()` 을 쓰지 않고 `LOCALAPPDATA` 를 읽으므로 그 환경변수도 임시 폴더로 패치한다. 후자가 빠지면 CI windows-latest 러너에서 격리가 되지 않는다.) 이 픽스처가 있으면 `main(["serve", ...])` 를 부르는 CLI 테스트가 실제 폴더 대신 임시 폴더에 설치한다. **실환경 실증은 관통에서** 한다: `~/Library/Fonts` 의 두 파일을 임시 폴더로 옮긴 뒤 `serve` 를 한 번 실행해 "설치했습니다" 안내와 파일 복원을 확인한다(되돌리기: 앱이 번들에서 다시 복사하므로 옮긴 파일은 삭제해도 된다. 실패하면 임시 폴더의 파일을 되돌린다).
...
- autouse 픽스처 `isolated_font_dirs(tmp_path, monkeypatch)`: `Path.home` 을 `tmp_path / "isolated_home"` 을 돌려주는 함수로, `LOCALAPPDATA` 환경변수를 그 아래 경로로, `installer._system_font_dirs` 를 `[]` 를 돌려주는 함수로 바꾼다(2026-09-05 정정: 가정 4 참조. 초안의 `_user_font_dir` 교체는 기존 플랫폼 분기 테스트와 충돌한다). docstring 에 사유(이월표 행)를 적는다.
...
2. **실행 스크립트**: 새 클론의 `SlideCaptain실행.command` 를 `open` 으로 실행해(Finder 더블클릭과 같은 경로) Terminal 창이 열리고 서버가 뜨고 브라우저가 열리는지, 이미 떠 있을 때 다시 실행하면 브라우저만 여는지, **Ctrl+C 를 누르면 서버가 멈추는지**(D2-3 최종 리뷰 반영 4753069 의 검증 대상. 이 항목이 없으면 관통이 그 결함을 놓친다), 창을 닫으면 서버가 멈추는지 확인한다. Gatekeeper 가 막으면 가정 2 를 정정하고 우회 안내를 README 에 적는다.
